...
// ─── Public static routes (must come before :id wildcards) ───────────────────

// GET /showrooms/featured
...
// ─── Dealer "my" routes (MUST be before /:id wildcards) ──────────────────────
...
router.post("/showrooms/upload", authMiddleware, upload.single("image"), async (req: AuthRequest, res): Promise<void> => {
  ...
  try {
    const isSafe = await checkImageSafety(req.file.buffer);
...
// GET /showrooms/my/cars — all cars for owner (all statuses)
...
// POST /showrooms/my/cars — publish a car linked to the dealer's showroom
...
// ─── Public wildcard routes (MUST come after static/my routes) ───────────────
...
// GET /showrooms/:id/cars — public approved cars only
router.get("/showrooms/:id/cars", async (req, res): Promise<void> => {
  ...
  res.json(cars.map(c => ({ ...c, primaryImage: imageMap[c.id] || null })));
});

// POST /showrooms/:id/rate
