...
import {
  ClipboardCheck, MapPin, Phone, MessageCircle, Star, ShieldCheck,
  Loader2, Search, Wrench,
} from "lucide-react";
...
function CenterCard({ center }: { center: any }) {
  ...
  const handleRate = async (score: number) => {
    ...
    if (rated) return;
    try {
      const res = await fetch(withApi(`/api/inspection-centers/${center.id}/rate`), {
        method: "POST",
  ...
  return (
    <div className={cn(
      ...
      center.isFeatured && "border-blue-300 ring-1 ring-blue-200"
    )}>
      <div className={cn(
        "px-4 py-3 flex items-start gap-3 border-b",
        center.isFeatured ? "bg-blue-50 border-blue-100" : "bg-muted/30"
      )}>
        <div className={cn(
          "w-12 h-12 rounded-xl overflow-hidden flex-shrink-0 flex items-center justify-center",
          center.isFeatured ? "bg-blue-100 border border-blue-200" : "bg-blue-500/10 border"
        )}>
          {center.logo
            ? <img src={withApi(center.logo)} alt={center.name} className="w-full h-full object-cover" />
            : <ClipboardCheck className={cn("w-6 h-6", center.isFeatured ? "text-blue-600" : "text-blue-500/70")} />
          }
        </div>
        <div className="min-w-0 flex-1">
          ...
          {center.isFeatured && (
            <span className="text-xs text-blue-600 font-semibold flex items-center gap-0.5">
              <Star className="w-3 h-3 fill-blue-500 text-blue-500" /> مركز مميز
            </span>
          )}
        </div>
      </div>
      ...
      <div className="p-4 space-y-2 flex-1">
        ...
        <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
          <MapPin className="w-3.5 h-3.5 shrink-0 text-blue-500/60" />
        ...
        {center.services && (
          <div className="flex flex-wrap gap-1 mt-2">
            {center.services.split(",").slice(0, 3).map((s: string, i: number) => (
      ...
      <div className="px-4 pb-4 flex gap-2 flex-wrap">
        {user && center.ownerUserId && user.id === center.ownerUserId ? (
          <button
            onClick={() => navigate(`/inspection-center/${center.id}`)}
            className="flex-1 flex items-center justify-center gap-1.5 bg-blue-600 text-white rounded-xl py-2 text-xs font-bold transition-colors hover:bg-blue-700"
          >
            <ClipboardCheck className="w-3.5 h-3.5" /> عرض صفحة مركزي
          </button>
        ) : (
          <>
            {center.phone && (
        ...
        <ShareSheet
          options={{ title: center.name, city: center.city, url: `${baseUrl}/inspection-center/${center.id}`, description: center.description }}
...
export default function InspectionCentersPage() {
  const [search, setSearch] = useState("");
  const [cityFilter, setCityFilter] = useState("");

  const { data: centers = [], isLoading } = useQuery<any[]>({
    queryKey: ["/inspection-centers"],
    queryFn: () => fetch(withApi("/api/inspection-centers")).then(r => r.json()),
    staleTime: 60_000,
  });
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
      <div className="relative overflow-hidden bg-gradient-to-l from-blue-700 to-blue-500 text-white px-4 pt-6 pb-5">
        <div className="relative z-[1] max-w-5xl mx-auto">
          <div className="flex items-center gap-3 mb-1">
            <ClipboardCheck className="w-7 h-7" />
            <h1 className="text-2xl font-extrabold tracking-tight">مراكز فحص السيارات</h1>
          </div>
          <p className="text-white/80 text-sm">مراكز فحص موثّقة في جميع المحافظات السورية</p>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 pt-4 pb-24">
        <div className="flex gap-2 mb-5">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="ابحث عن مركز فحص..."
              className="w-full border rounded-xl pr-9 pl-3 py-2.5 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/30"
            />
          </div>
          <select
            value={cityFilter}
            onChange={e => setCityFilter(e.target.value)}
            className="border rounded-xl px-3 py-2.5 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/30"
          >
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <ClipboardCheck className="w-14 h-14 mx-auto mb-3 text-muted-foreground/30" />
            <p className="text-muted-foreground font-bold">لا توجد مراكز فحص</p>
          </div>
        ) : (
          <>
            {featured.length > 0 && (
