...
import {
  ...
  ClipboardCheck, ChevronRight, PenLine, Camera, Settings2, Save, X,
} from "lucide-react";
...
function ImageUploadBtn({ onUploaded, className, children }: {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch(withApi("/api/inspection-centers/upload"), {
        method: "POST",
      ...
      <div className="bg-background rounded-t-2xl sm:rounded-2xl w-full max-w-lg p-6 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div><label className="text-xs text-muted-foreground mb-1 block">اسم المركز</label>
            <input ref={nameRef} defaultValue={center.name} className="w-full border rounded-xl px-3 py-2.5 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/30" /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs text-muted-foreground mb-1 block">هاتف</label>
              <input ref={phoneRef} type="tel" defaultValue={center.phone ?? ""} className="w-full border rounded-xl px-3 py-2.5 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/30" dir="ltr" /></div>
            <div><label className="text-xs text-muted-foreground mb-1 block">واتساب</label>
              <input ref={waRef} type="tel" defaultValue={center.whatsapp ?? ""} className="w-full border rounded-xl px-3 py-2.5 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/30" dir="ltr" /></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs text-muted-foreground mb-1 block">المدينة</label>
              <input ref={cityRef} defaultValue={center.city ?? ""} className="w-full border rounded-xl px-3 py-2.5 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/30" /></div>
            <div><label className="text-xs text-muted-foreground mb-1 block">العنوان</label>
              <input ref={addressRef} defaultValue={center.address ?? ""} className="w-full border rounded-xl px-3 py-2.5 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/30" /></div>
          </div>
          <div><label className="text-xs text-muted-foreground mb-1 block">الخدمات (افصل بفاصلة)</label>
            <textarea ref={servicesRef} defaultValue={center.services ?? ""} rows={2} placeholder="فحص شامل، فحص بالكمبيوتر، تقييم قبل الشراء" className="w-full border rounded-xl px-3 py-2.5 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/30 resize-none" /></div>
          <div><label className="text-xs text-muted-foreground mb-1 block">نبذة عن المركز</label>
            <textarea ref={descRef} defaultValue={center.description ?? ""} rows={3} className="w-full border rounded-xl px-3 py-2.5 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/30 resize-none" /></div>
        </div>
        <button onClick={save} disabled={saving} className="mt-5 w-full flex items-center justify-center gap-2 bg-blue-600 text-white rounded-xl py-3 font-bold text-sm disabled:opacity-60">
...
export default function InspectionCenterPage() {
  ...
  const { data: center, isLoading } = useQuery<any>({
    queryKey: ["/inspection-centers", id],
    queryFn: () => fetch(withApi(`/api/inspection-centers/${id}`)).then(r => r.json()),
    enabled: !isNaN(id),
  });
  ...
  const services = center.services ? center.services.split(",").map((s: string) => s.trim()).filter(Boolean) : [];

  const handleImageUpload = async (field: "logo" | "coverImage", url: string) => {
    try {
      await apiRequest("/api/inspection-centers/my", "PATCH", { [field]: url });
      qc.invalidateQueries({ queryKey: ["/inspection-centers", id] });
  ...
  const handleRate = async (score: number) => {
    ...
    try {
      const res = await fetch(withApi(`/api/inspection-centers/${id}/rate`), {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
      {editOpen && isOwner && (
        <EditModal center={center} onClose={() => setEditOpen(false)} onSaved={() => qc.invalidateQueries({ queryKey: ["/inspection-centers", id] })} />
      )}

      {/* Cover */}
      <div className="relative w-full h-52 bg-gradient-to-br from-blue-700 via-blue-600 to-blue-500 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          ...
          {center.isFeatured && <span className="flex items-center gap-1 bg-blue-500/90 text-white text-xs font-bold px-2.5 py-1 rounded-full backdrop-blur-sm"><Star className="w-3 h-3 fill-white" /> مميّز</span>}
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="relative -mt-14 mb-5">
          <div className="bg-card border rounded-2xl shadow-lg p-5">
            <div className="flex items-start gap-4 mb-4">
              <div className="relative flex-shrink-0 w-20 h-20">
                <div className="w-20 h-20 rounded-xl border-2 border-border bg-muted shadow-sm overflow-hidden">
                  {center.logo
                    ...
                    : <div className="w-full h-full bg-blue-500/10 flex items-center justify-center"><ClipboardCheck className="w-9 h-9 text-blue-500/60" /></div>
                  }
                </div>
                {isOwner && (
                  <ImageUploadBtn onUploaded={url => handleImageUpload("logo", url)}
                    className="absolute -bottom-1.5 -left-1.5 w-7 h-7 rounded-full bg-blue-600 flex items-center justify-center shadow-md hover:bg-blue-700 border-2 border-background">
              ...
              <div className="flex-1 min-w-0 pt-1">
                ...
                <div className="flex items-center gap-1.5 mt-1.5">
                  <ShareSheet
                    options={{ title: center.name, city: center.city, url: `${baseUrl}/inspection-center/${id}`, description: center.description }}
                  ...
                  />
                  {isOwner && (
                    <button onClick={() => setEditOpen(true)} className="flex items-center gap-1 bg-blue-500/10 hover:bg-blue-500/20 text-blue-700 border border-blue-200 rounded-full px-2.5 py-1 text-xs font-bold">
            ...
            {services.length > 0 && (
              <div className="mb-4">
                <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">خدمات الفحص</h3>
                <div className="flex flex-wrap gap-2">
                  {services.map((s: string, i: number) => (
                    <span key={i} className="flex items-center gap-1 bg-blue-50 text-blue-700 border border-blue-100 rounded-full px-3 py-1 text-sm font-medium">
                      <ClipboardCheck className="w-3.5 h-3.5" /> {s}
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {center.phone && (
                    <a href={`tel:${center.phone}`} className="flex items-center justify-center gap-2 bg-muted/60 hover:bg-muted border rounded-xl py-2.5 font-bold text-sm">
                      <Phone className="w-4 h-4 text-blue-600" /> اتصال
                    </a>
                  )}
                  {center.whatsapp && (
              ...
              {isOwner && (
                <button onClick={() => navigate("/inspection-center/manage")}
                  className="w-full flex items-center justify-center gap-2 bg-blue-600 text-white rounded-xl py-2.5 font-bold text-sm hover:bg-blue-700 active:scale-[0.98]">
