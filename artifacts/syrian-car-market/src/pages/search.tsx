...
import { Car, Wrench, Trash2, CalendarDays, Filter, SlidersHorizontal, Search as SearchIcon, X, Plus, ShoppingCart, MapPin, Tag, Phone } from "lucide-react";
...
const WA_MSG = encodeURIComponent("مرحباً، رأيت إعلانك على MARKLET. هل ما زال متوفراً؟");
...
function UnifiedCard({ item, onClick }: { item: UnifiedResult; onClick?: () => void }) {
  ...
  const phone = item.sellerPhone?.trim() || null;
  const phoneClean = phone ? formatPhone(phone) : null;
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        {/* Contact buttons */}
        <div className="flex gap-2 mt-1" onClick={handleContactClick}>
          {phoneClean ? (
            <>
              <a
                href={`https://wa.me/${phoneClean.replace("+", "")}?text=${WA_MSG}`}
                target="_blank"
                rel="noopener noreferrer"
                onClick={e => e.stopPropagation()}
                className="flex-1 inline-flex items-center justify-center gap-1.5 h-9 rounded-xl text-xs font-bold bg-green-600 hover:bg-green-700 text-white transition-colors no-underline"
              >
              ...
              <a
                href={`tel:${phoneClean}`}
                onClick={e => e.stopPropagation()}
                className="flex-1 inline-flex items-center justify-center gap-1.5 h-9 rounded-xl text-xs font-bold border-2 border-primary text-primary hover:bg-primary hover:text-white transition-colors no-underline"
              >
            ...
            <button
              onClick={e => { e.stopPropagation(); window.location.href = `/cars/${item.id}`; }}
              className="w-full h-9 rounded-xl text-xs font-bold border-2 border-primary/30 text-primary hover:bg-primary/5 transition-colors flex items-center justify-center gap-1.5"
            >
              <Phone className="w-3.5 h-3.5" />
              عرض معلومات التواصل
            </button>
