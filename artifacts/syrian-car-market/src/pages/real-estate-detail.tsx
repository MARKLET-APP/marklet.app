...
export default function RealEstateDetail() {
  ...
  /* Filter out empty strings, null, and blob: URLs that may have been stored accidentally */
  ...
  const markBroken = (idx: number) =>
    setBrokenImgs(prev => new Set([...prev, idx]));

  /* Visible images = those not marked broken yet */
  const visibleImages = images.filter((_, i) => !brokenImgs.has(i));

  return (
    /* h-full flex flex-col — fills #app-main exactly; contact bar stays at bottom without position:fixed */
    <div className="h-full flex flex-col" dir="rtl">
      ...
      {/* Back bar — flex-shrink-0 so it never scrolls away */}
      <div className="flex-shrink-0 sticky top-0 z-10 bg-background/90 backdrop-blur border-b px-4 py-3 flex items-center gap-3">
      ...
      {/* Scrollable content — flex-1 so it fills the space between back bar and contact bar */}
          ...
          <div className="bg-card border-b">
            ...
            <div className="relative aspect-video bg-muted">
              {visibleImages.length > 0 ? (
                <>
                  <img
                    key={visibleImages[Math.min(activeImg, visibleImages.length - 1)]}
                    src={visibleImages[Math.min(activeImg, visibleImages.length - 1)]}
                    alt={item.title}
                    className="w-full h-full object-cover"
                    onError={() => {
                      /* find original index in images array for the current visible image */
                      const src = visibleImages[Math.min(activeImg, visibleImages.length - 1)];
                  ...
                  <div className="absolute top-3 end-3 bg-black/50 text-white text-xs px-2 py-1 rounded-full">
                    {Math.min(activeImg, visibleImages.length - 1) + 1} / {visibleImages.length}
                  </div>
                </>
              ) : (
      ...
      {/* Contact bar — flex-shrink-0 at bottom of flex column, NO position:fixed */}
      <div className="flex-shrink-0 bg-background/95 backdrop-blur-sm border-t shadow-lg px-4 py-3 flex gap-2">
        {phone ? (
          <>
            <Button
