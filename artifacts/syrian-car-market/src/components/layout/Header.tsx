...
import { Bell, Menu, User, Crown, MessageSquare } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 w-full glass-panel border-b">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden sm:flex items-center gap-6">
          ...
          {user?.role === "admin" && (
            <Link href="/admin" className="text-sm font-semibold text-accent hover:text-accent/80 transition-colors">
              لوحة التحكم
            </Link>
          )}
        </nav>
